...
import { book, golbal } from "./sotre/store_book";
...
/// 解析
const parse = (str: string):book.StoreBookData["word_list"]=>{
   ...
   const word_list:book.StoreBookData["word_list"] = {};
   matches?.forEach(v=>{
      if(v.includes("|")){ // <word>|<note>| 型
         ...
         if(matches){
            ...
            golbal.store_golbal.set_note(word,note);
         }
